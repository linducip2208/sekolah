...
use App\Jobs\NotifyParentPaymentReceivedJob;
...
class PaymentService
{
    ...
    public function handleWebhook(PaymentProvider $provider, array $headers, string $rawBody): PaymentWebhookLog
    {
        ...
        $replayedLog = PaymentWebhookLog::where('payment_provider_id', $provider->id)
            ->where('payload_hash', $payloadHash)
            ->where('id', '!=', $log->id)
            ->whereIn('processing_status', [
                PaymentWebhookLog::PROCESSING_PROCESSED,
                PaymentWebhookLog::PROCESSING_DUPLICATE,
            ])
            ->latest('id')
            ->first();
        ...
        if ($replayedLog) {
            $log->update([
                'signature_status' => PaymentWebhookLog::SIGNATURE_VALID,
                'processing_status' => PaymentWebhookLog::PROCESSING_DUPLICATE,
                'error_message' => 'Replay detected; callback already processed by log '.$replayedLog->id.'.',
            ]);
    ...
    public function applyStatusUpdate(PaymentTransaction $tx, array $event): PaymentTransaction
    {
        return DB::transaction(function () use ($tx, $event) {
            $tx = PaymentTransaction::withoutGlobalScopes()
                ->whereKey($tx->id)
                ->lockForUpdate()
                ->firstOrFail();

            if ($tx->status === PaymentTransaction::STATUS_PAID && $event['status'] === 'paid') {
                return $tx;
            }

            if ($tx->isTerminal() && $event['status'] === 'paid') {
                return $tx;
            }
