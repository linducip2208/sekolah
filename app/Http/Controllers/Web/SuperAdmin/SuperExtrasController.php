...
class SuperExtrasController extends Controller
{
    ...
    public function triggerBackup(): RedirectResponse
    {
        ...
        // Never report a synthetic file as a successful database backup.
        if (! function_exists('exec')) {
            return back()->withErrors('Backup gagal: fungsi exec tidak tersedia di server.');
        }
        ...
        if ($exit !== 0 || ! file_exists($file) || filesize($file) === 0) {
            ...
            return back()->withErrors('Backup gagal: mysqldump tidak tersedia atau proses dump mengembalikan error.');
        }
    ...
    public function downloadBackup(string $name): BinaryFileResponse
    {
        ...
        if (! preg_match('/^backup-[0-9_\-]+\.sql$/i', $name)) {
            abort(404);
        }
    ...
    public function restoreBackup(Request $request, string $name): RedirectResponse
    {
        if (! preg_match('/^(backup|restore-uploaded)-[0-9_\-]+\.sql$/i', $name)) {
            abort(404);
        }
    ...
    {
        $from = $request->from ? Carbon::parse($request->from)->startOfDay() : now()->subMonths(12)->startOfMonth();
    ...
    {
        $from = $request->from ? Carbon::parse($request->from) : now()->subMonths(12)->startOfMonth();
        $to = $request->to ? Carbon::parse($request->to) : now()->endOfDay();
