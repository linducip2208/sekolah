@props(['theme' => [], 'landing' => []])
@php
    // Do not publish invented endorsements; only approved real entries may render.
    $testimonials = collect($landing['testimonials'] ?? [])
        ->reject(fn (array $testimonial): bool => (bool) ($testimonial['placeholder'] ?? false))
        ->values();
@endphp
