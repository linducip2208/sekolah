...
@section('title', 'Kategori Bank Soal')
@section('sidebar')@include('school-admin.partials.sidebar')@endsection
@section('content')
<div class="mb-7 flex justify-between items-end"><div>
<div class="elite-kicker mb-2">Categoriae Quaestionum</div>
<h1 class="elite-h1 text-3xl ink-primary mb-2">Kategori Bank Soal</h1><div class="elite-rule"></div></div>
<a href="{{ route('admin.qbank.items.index') }}" class="btn-elite-ghost">Daftar Soal →</a></div>

<div class="grid lg:grid-cols-3 gap-6">
<div class="lg:col-span-1"><div class="bg-white border border-rule p-6 sticky top-6">
<form method="POST" action="{{ route('admin.qbank.categories.store') }}" class="space-y-3">@csrf
<input name="name" required maxlength="200" placeholder="Aljabar/Geometri/dll" class="w-full border-2 border-rule px-3 py-2 font-serif text-sm">
...
<td class="px-4 py-3 text-xs">{{ $c->subject?->name }}</td>
<td class="px-4 py-3 text-right"><form method="POST" action="{{ route('admin.qbank.categories.destroy', $c) }}" class="inline" onsubmit="return confirm('Hapus?')">@csrf @method('DELETE')<button class="text-xs text-red-700 hover:underline">Hapus</button></form></td>
