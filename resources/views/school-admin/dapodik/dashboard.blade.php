...
<div class="space-y-6">
    ...
    <div class="bg-white rounded-lg shadow p-5">
        ...
        <div class="mt-4 flex gap-2">
            <a href="{{ route('admin.dapodik.config.index') }}" class="btn-brand text-sm">Edit Config</a>
            <a href="{{ route('admin.dapodik.config.index') }}" class="btn-brand text-sm" style="background-color: #16A34A">Preview Sync</a>
            <a href="{{ route('admin.dapodik.config.index') }}" class="btn-brand text-sm" style="background-color: #6366F1">Konfigurasi Mapping</a>
        </div>
    </div>
    ...
    <div class="bg-white rounded-lg shadow">
        ...
        <table class="w-full text-sm">
            ...
            <tbody class="divide-y">
                @forelse($recentRuns as $s)
                    <tr>
                        <td class="px-4 py-2 text-xs">{{ $s->created_at->format('d/m H:i') }}</td>
                        <td class="px-4 py-2">{{ $s->direction }}</td>
                        <td class="px-4 py-2">{{ $s->entity_type }}</td>
                        <td class="px-4 py-2">{{ $s->total }}</td>
                        <td class="px-4 py-2 text-green-600">{{ $s->inserted + $s->updated + $s->unchanged }}</td>
                        <td class="px-4 py-2 text-red-600">{{ $s->failed + $s->conflicted }}</td>
                        <td class="px-4 py-2">
