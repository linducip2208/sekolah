...
<div class="flex gap-2 mb-5">
    <a href="{{ route('admin.classroom.assignments.create') }}" class="btn-elite text-xs">+ Tugas Baru</a>
</div>
...
<td class="px-3 py-3 text-right space-x-1">
    <a href="{{ route('admin.classroom.assignments.edit', $a) }}" class="text-xs underline ink-secondary hover:ink-accent">Edit</a>
    <a href="{{ route('admin.classroom.assignments.submissions', $a) }}" class="text-xs underline text-blue-700">Nilai</a>
    <form method="POST" action="{{ route('admin.classroom.assignments.destroy', $a) }}" class="inline" onsubmit="return confirm('Hapus?')">@csrf @method('DELETE')<button class="text-xs text-red-700 hover:underline">Hapus</button></form>
</td>
