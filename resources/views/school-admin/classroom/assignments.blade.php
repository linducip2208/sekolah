...
<form method="POST" action="{{ route('admin.classroom.assignments.store') }}" class="px-5 py-5 border-t border-rule grid md:grid-cols-2 gap-3">@csrf
...
<td class="px-3 py-3 text-right"><form method="POST" action="{{ route('admin.classroom.assignments.destroy', $a) }}" class="inline" onsubmit="return confirm('Hapus?')">@csrf @method('DELETE')<button class="text-xs text-red-700 hover:underline">Hapus</button></form></td>
