...
<div class="max-w-lg mx-auto px-4 py-12 text-center">
    <div class="bg-white border border-rule shadow-sm p-8">
        ...
        <h2 class="font-display text-2xl font-bold text-stone-800 mb-3">Terima Kasih, {{ $visit->visitor?->name }}!</h2>
        ...
        <div class="bg-stone-50 border border-stone-200 rounded-xl p-4 text-left text-sm space-y-2 mb-6">
            <div><span class="font-semibold">Nama:</span> {{ $visit->visitor?->name }}</div>
            <div><span class="font-semibold">Tujuan:</span> {{ $visit->purpose }}</div>
            <div><span class="font-semibold">Kedatangan:</span> {{ $visit->expected_arrival?->format('d M Y H:i') ?? '-' }}</div>
            <div><span class="font-semibold">Badge:</span> {{ $visit->badge_number ?? 'Diterbitkan saat check-in' }}</div>
            <div><span class="font-semibold">Token QR:</span> <code class="text-xs bg-stone-200 px-2 py-0.5 rounded">{{ substr($visit->qr_token, 0, 16) }}...</code></div>
        </div>
