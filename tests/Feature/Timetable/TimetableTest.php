...
test('timetable rejects foreign academic references', function () {
    Sanctum::actingAs($this->admin);
    $foreignSchool = School::factory()->create();
    $foreignTeacher = User::factory()->create(['school_id' => $foreignSchool->id]);
...
test('bulk timetable rejects an in-batch class conflict', function () {
    Sanctum::actingAs($this->admin);
    $teacher2 = User::factory()->create(['school_id' => $this->school->id]);
    $subject2 = Subject::create([
        'school_id' => $this->school->id,
    ...
    $this->postJson('/api/v1/timetable/bulk', [
        'class_section_id' => $this->classSection->id,
        'slots' => [
            ['subject_id' => $this->subject->id, 'teacher_id' => $this->teacher->id, 'day_of_week' => 5, 'start_time' => '07:00', 'end_time' => '08:00'],
            ['subject_id' => $subject2->id, 'teacher_id' => $teacher2->id, 'day_of_week' => 5, 'start_time' => '07:30', 'end_time' => '08:30'],
        ],
    ])->assertStatus(422);
